...
export default function SuturingPage(){
  ...
  useEffect(()=>{const abort=new AbortController();fetch(`${motionBase}.json`,{signal:abort.signal}).then(r=>{if(!r.ok)throw new Error('Suturing recording is unavailable');return r.json() as Promise<StitchMotion>;}).then(d=>{if(d.schema_version!=='soren.stitch.v1'||!d.frames?.length)throw new Error('Unsupported recording');setMotion(d);}).catch(e=>{if(e.name!=='AbortError')setError(e.message);});return()=>abort.abort();},[motionBase]);
