...
export function roomContext(motion:StitchMotion,ready:()=>void,status:(message:string)=>void,cancelled:()=>boolean){
  ...
  Promise.allSettled([
    ...
    motion.presentation_assets?.recording_specific && !motion.presentation_assets.robot_motion ? Promise.resolve(null) : loader.loadAsync('/models/robot/panda.glb'),
    motion.presentation_assets?.robot_motion ? Promise.resolve(motion.presentation_assets.robot_motion) : motion.presentation_assets?.recording_specific ? Promise.resolve(null) : fetch(motion.presentation_assets?.robot_fit ?? '/motion/stitch-robot.json').then(r=>{if(!r.ok)throw new Error('Suturing robot fit unavailable');return r.json() as Promise<{frames:RobotFrame[];seed:number;checkpoint_sha256:string}>;}),
  ]).then(results=>{
    const errors=results.filter(r=>r.status==='rejected');
    if(cancelled()||errors.length){
      results.forEach((r,i)=>{if(r.status==='fulfilled'&&i<4){if(r.value instanceof T.Texture)r.value.dispose();else if (r.value && "scene" in r.value) disposeTree(r.value.scene);}});
    ...
    if(fit && (fit.seed!==motion.scene.seed||fit.checkpoint_sha256!==motion.result.checkpoint_sha256||fit.frames.length!==motion.frames.length)){
      disposeTree(head.scene);if(panda)disposeTree(panda.scene);color.dispose();normal.dispose();status('Robot fit does not match this recording. Rebuild the suturing presentation.');return;
    }
    ...
    if(!panda || !fit){status('Scanned patient · measured instrument motion');ready();return;}
