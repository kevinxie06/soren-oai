...
export function roomContext(motion:StitchMotion,ready:()=>void,status:(message:string)=>void,cancelled:()=>boolean){
  ...
  Promise.allSettled([
    ...
    fetch(motion.presentation_assets?.robot_fit ?? '/motion/stitch-robot.json').then(r=>{if(!r.ok)throw new Error('Suturing robot fit unavailable');return r.json();}),
  ]).then(results=>{
    const errors=results.filter(r=>r.status==='rejected');
    if(cancelled()||errors.length){
      results.forEach((r,i)=>{if(r.status==='fulfilled'&&i<4){if(r.value instanceof T.Texture)r.value.dispose();else disposeTree(r.value.scene);}});
